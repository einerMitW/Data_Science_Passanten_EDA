...
#' [summe pro tag]
#' @description
#' [die Funktion erstellt eine Datumsspalte und Gruppiert dann nach Datum, wochentag und location. Dann wird für jewails die Summe des einzelnen Tages berechnet in der Spalte Tagessumme]
#'
#' @param passanten
#' @return
#' [df tagessumme als long format welcher die Summe der Passanten für jeden einzlenen Tag im Jahr enthält]
...
#' @return
#' []
durchschnittlicher_tageswert_location_tabelle <- durchschnittlicher_tageswert_location_plot %>%
   pivot_wider(
      names_from = wochentag,
      values_from = durchschnitt_tag
   ) %>%
   mutate(
      Gesamtwert = rowSums(across(where(is.numeric)), na.rm = TRUE)
   )
durchschnittlicher_tageswert_location_tabelle
```
````{r}
#' [summe pro tag]
#' @description
...
#' @return
#' [df durchnittlicher_tageswert der Passanten pro Messtation und Wochentag in long format]
durchschnittlicher_tageswert_gesamt_plot <- gesamttagessumme %>%
   mutate(
    wochentag = factor(wochentag, levels = tage_vektor)
   )%>%
   group_by(wochentag)%>%
   ...
   ungroup()
durchschnittlicher_tageswert_gesamt_plot

````
## Aufgabe 5

````{r}
#Berechne zuerst den median je Stunde in Wide Format pro Location
passantenanzahl_stunden <- passanten %>%
...
mutate(gesamtsumme = rowSums(across(where(is.numeric)), na.rm = TRUE))
passantenanzahl_stunden

````

````{r}
#Umformen des Wide Format in Long zum Plotten
passantenanzahl_plot <- passantenanzahl_stunden %>%
pivot_longer(cols = -stunde,
names_to = "location_name",
values_to = "gesamtsumme"
)
#passantenanzahl_plot
ggplot(data = passantenanzahl_plot, aes(x = stunde, y = gesamtsumme, color = location_name)) +
geom_line(linewidth = 1) + # Zeichnet die Linien
geom_point() +
 # Fügt die Datenpunkte hinzu
# Titel und Achsenbeschriftungen
labs(title = "gesamtsumme über die Wochentage",
x = "Wochentage",
y = "Summe",
color = "location")
