...
````{r}
#Berechne Durchschnittlichen Tageswert
...
durchschnittlicher_tageswert

#Berechne Gesamtsumme
gesamtsumme_tageswert <- durchschnittlicher_tageswert %>%
   pivot_wider(
      names_from = location_name,
      values_from = durchschnitt
   )%>%
   mutate(gesamtsumme = rowSums(across(where(is.numeric)), na.rm = TRUE))
gesamtsumme_tageswert
...
min <- min(gesamtsumme_tageswert$gesamtsumme)
max <- max(gesamtsumme_tageswert$gesamtsumme)

#rbind bindet alle werte die man übergibt als neue Zeile auf einen neune df
spider_plot_data <- rbind(rep(max, df_weite), rep(min, df_weite), spider_plot_data, gesamtsumme_tageswert$gesamtsumme)
spider_plot_data
````
...
ggplot(data = gesamtsumme_tageswert, aes(x = wochentag, y = gesamtsumme)) +
  geom_line(linewidth = 1) +  # Zeichnet die Linien
  geom_point() +              # Fügt die Datenpunkte hinzu
...
Je Stunde
Tagesverlauf
Nach Messtellen und Gesamtsumme_Messtellen

Problem:
1. df transformieren in df nach gruppe nach summe
````{r}
korrekte_tage <- c("Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", "Samstag", "Sonntag")
